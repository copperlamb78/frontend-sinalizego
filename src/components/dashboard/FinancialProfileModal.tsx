...
// Inner Form component that only mounts and computes when modal is open
const FinancialProfileForm: React.FC<{
  ...
  defaultEmail?: string;
  defaultPhone?: string;
}> = ({ onClose, company, defaultName, defaultEmail, defaultPhone }) => {
  const queryClient = useQueryClient();
  const { user } = useAuth();
  const [isSearchingCep, setIsSearchingCep] = useState(false);

  // Initial values computed once on mount
  const initialValues: FinancialProfileFormData = useMemo(() => {
    return {
      name: defaultName || company?.businessName || user?.name || '',
      email: defaultEmail || user?.email || '',
      cpfCnpj: user?.cpfCnpj || '',
      birthDate: '',
      companyType: undefined,
      mobilePhone: defaultPhone || company?.whatsapp || user?.phone || '',
      incomeValue: 3500,
  ...
  }, [company, defaultName, defaultEmail, defaultPhone, user]);
  ...
  // Isolated watchers - only re-render the specific fields that need them
  const rawCpfCnpj = useWatch({ control, name: 'cpfCnpj' }) || '';
  const isCnpj = useMemo(() => rawCpfCnpj.replace(/\D/g, '').length > 11, [rawCpfCnpj]);

  const selectedPixType = useWatch({ control, name: 'pixAddressKeyType' }) || 'CPF';
  const selectedCompanyType = useWatch({ control, name: 'companyType' }) || '';

  // Handle CEP auto-fill via BrasilAPI (reads from getValues, 0 re-renders while typing)
  const handleCepBlur = useCallback(async () => {
    const rawCep = getValues('postalCode')?.replace(/\D/g, '');
    if (rawCep && rawCep.length === 8) {
      setIsSearchingCep(true);
      try {
        const addressData = await cepService.fetchAddressByCep(rawCep);
        if (addressData) {
          setValue('address', addressData.street || '', { shouldValidate: true });
          setValue('province', addressData.neighborhood || '', { shouldValidate: true });
        }
      } catch {
        // ignore CEP fetch errors
      } finally {
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {isCnpj ? (
            <div className="space-y-1">
              <Select
                label="Tipo Societário da Empresa (CNPJ)"
                value={selectedCompanyType}
                onChange={(val) => setValue('companyType', val as any, { shouldValidate: true })}
            ...
            <div className="space-y-1">
              <Input
                ...
                type="date"
                leftIcon={<Calendar className="w-4 h-4 text-teal-400" />}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="space-y-1">
            <Select
              ...
              onChange={(val) => setValue('pixAddressKeyType', val as any, { shouldValidate: true })}
...
// Main Export: Pure wrapper that mounts the form ONLY when isOpen === true
export const FinancialProfileModal: React.FC<FinancialProfileModalProps> = ({
  ...
  return (
    <Modal
      ...
      size="lg"
    >
      <FinancialProfileForm onClose={onClose} {...props} />
    </Modal>
  );
};
